...
interface ShotlistAIEditPanelProps {
  onSubmit: (instruction: string) => Promise<void>
  isLoading?: boolean
}

export function ShotlistAIEditPanel({ onSubmit, isLoading = false }: ShotlistAIEditPanelProps) {
    ...
    <div className="w-full rounded-xl border border-[#2A2A2A] bg-[#111111] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">Edit Shotlist with AI</span>
        </div>
        <motion.div
          ...
          >
            <div className="px-4 pb-4 pt-1 flex flex-col gap-3 border-t border-[#1F1F1F]">
              <p className="text-xs text-muted-foreground/60 pt-1">
                Describe changes to apply across the entire shotlist — e.g. &quot;change all shot
                types to CU&quot; or &quot;add cinematic style to every row&quot;.
              </p>
              <textarea
                value={instruction}
                onChange={(e) => setInstruction(e.target.value)}
                onKeyDown={handleKeyDown}
                placeholder="Describe what you'd like to change in the shotlist..."
                rows={3}
                disabled={isLoading}
                className={`
